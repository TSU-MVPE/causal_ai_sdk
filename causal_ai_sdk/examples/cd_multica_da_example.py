"""Example: MultiCa causal discovery followed by Decision Analysis (explain).

This example runs the full flow:
1. Create session
2. Upload MultiCa data and run matching
3. Submit MultiCa task and wait for CD result
4. Build current_observation and targets from CD result
...
def _build_current_observation_and_targets(
    ...
    Args:
        cd_result (Dict[str, Any]): CD result with training_data, feature_names, feature_metadata.
...
async def run_multica_da_workflow(client: CausalAIClient, test_data_dir: Path) -> bool:
    """Run MultiCa CD then DA explain; cleanup in finally.
    ...
    """
    multica_path = test_data_dir / "cd" / "multica_datasets.json"
    if not multica_path.exists():
        print(f"  [ERROR] MultiCa file not found: {multica_path}")
        return False
    ...
    try:
        ...
        print("  Step 2: Get CD Upload URL")
        print("  Step 3: Upload MultiCa Data")
        uploaded_data = await client.cd.upload_data_for_multica(session_uuid, multica_path)
        print("  [OK] CD upload URL obtained")
        print("  [OK] MultiCa datasets uploaded")

        print("  Step 4: MultiCa Matching")
        ...
        async with httpx.AsyncClient() as http_client:
            ...
            cd_result = resp.json()
        if cd_result.get("status") != "succeeded" or cd_result.get("mode") != "multica":
            print("  [ERROR] CD result not succeeded or not multica mode")
            return False
        ...
        if session_uuid:
            ...
            try:
                ...
                pass
        print("  [OK] Session and multica matching cleaned up")
